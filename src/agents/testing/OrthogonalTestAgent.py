...
class OrthogonalTestAgent(BaseAgent):
    """
    正交测试智能体，专精参数组合的正交测试用例生成
    """
    def __init__(
        self,
        model: BaseModel,
        ...
        super().__init__(
            ...
            enabled=enabled,
            agent_name="正交测试智能体",
            prompt_module_path="testing.orthogonal_array"
        )
    ...
    def _process_response(self, response: str) -> Dict[str, Any]:
        ...
        if json_match:
            try:
                ...
                return {
                    ...
                    "orthogonal_array": result.get("orthogonal_array", {}),
        ...
        # 提取参数分析
        param_analysis_match = re.search(r'(?:参数分析|输入参数分析)(?:[：:])?\s*(.*?)(?:正交数组|$)', response, re.DOTALL)
        param_analysis = param_analysis_match.group(1).strip() if param_analysis_match else ""
        
        # 提取正交数组
        orthogonal_array_match = re.search(r'(?:正交数组|正交表)(?:[：:])?\s*(.*?)(?:测试用例设计|$)', response, re.DOTALL)
        orthogonal_array = orthogonal_array_match.group(1).strip() if orthogonal_array_match else ""
