...
class CauseEffectAgent(BaseAgent):
    """
    因果图测试智能体，专精因果关系测试用例生成
    """
    def __init__(
        self,
        model: BaseModel,
        ...
        super().__init__(
            ...
            enabled=enabled,
            agent_name="因果图测试智能体",
            prompt_module_path="testing.cause_effect"
        )
    ...
    def _process_response(self, response: str) -> Dict[str, Any]:
        ...
        # 提取因分析
        cause_analysis_match = re.search(r'(?:因分析|原因分析|输入条件分析)(?:[：:])?\s*(.*?)(?:果分析|结果分析|输出结果分析|$)', response, re.DOTALL)
        cause_analysis = cause_analysis_match.group(1).strip() if cause_analysis_match else ""
        
        # 提取果分析
        effect_analysis_match = re.search(r'(?:果分析|结果分析|输出结果分析)(?:[：:])?\s*(.*?)(?:因果关系|$)', response, re.DOTALL)
        effect_analysis = effect_analysis_match.group(1).strip() if effect_analysis_match else ""
        
        # 提取因果关系
        relationship_match = re.search(r'(?:因果关系|关系分析)(?:[：:])?\s*(.*?)(?:测试用例设计|$)', response, re.DOTALL)
        relationships = relationship_match.group(1).strip() if relationship_match else ""
