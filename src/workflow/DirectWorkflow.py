...
class DirectWorkflow(BaseWorkflow):
    ...
    def __init__(
        ...
        verbose: int = 1,
        web_search: bool = False,  # Direct 不需要网络搜索
        docker_execution: bool = False,  # Direct 不需要 Docker
        start_index: int = 0,
    ):
        super().__init__(
            model=model,
    ...
    def _generate_code(self, item: Dict[str, Any]) -> Dict[str, Any]:
        """
        使用 Direct prompts + CodeAgent 生成代码
        ...
        if self.verbose >= VERBOSE_FULL:
            print(f"\nDirect Prompt 长度: {len(messages[0]['content'])} 字符")
