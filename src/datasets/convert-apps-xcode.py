...
test_set = read_jsonl("../../data/APPS/test.jsonl")
...
# Randomly choose 60 problems
codeforces_dataset_60 = codeforces_dataset.sample(n=min(60, len(codeforces_dataset)), random_state=1, replace=False)
print(len(codeforces_dataset_60))

codeforces_dataset_60.reset_index(drop=True, inplace=True)
...
# Randomly choose 180 problems
interview_dataset_180 = interview_dataset.sample(
    n=min(180, len(interview_dataset)), random_state=1, replace=False)
print(len(interview_dataset_180))

interview_dataset_180.reset_index(drop=True, inplace=True)
...
# Randomly choose 60 problems
introductory_dataset_60 = introductory_dataset.sample(
    n=min(60, len(introductory_dataset)), random_state=1, replace=False)
print(len(introductory_dataset_60))

introductory_dataset_60.reset_index(drop=True, inplace=True)

selected_df = pd.concat([introductory_dataset_60, interview_dataset_180, codeforces_dataset_60], ignore_index=True)
...
def get_test_cases(input, output):
    ...
    # 确保 output 是字符串列表
    if type(output) == list:
        output_list = [str(x) for x in output]
    else:
...
write_jsonl("../../data/APPS/selected300.jsonl", selected_datasets)
